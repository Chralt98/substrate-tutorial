...
fn create_handles_insufficient_balance() {
	new_test_ext().execute_with(|| {
		assert_noop!(
			SimpleCrowdfund::create(Origin::signed(1337), 2, 1000, 9),
...
fn contribute_works() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(SimpleCrowdfund::contribute(Origin::signed(1), 0, 49));
...
fn contribute_handles_basic_errors() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			SimpleCrowdfund::contribute(Origin::signed(1), 0, 49),
			Error::<Test>::InvalidIndex
		);
		// Cannot contribute below minimum contribution
		assert_noop!(
			SimpleCrowdfund::contribute(Origin::signed(1), 0, 9),
			Error::<Test>::ContributionTooSmall
		);

		// Set up a crowdfund
		assert_ok!(SimpleCrowdfund::create(Origin::signed(1), 2, 1000, 9));
		assert_ok!(SimpleCrowdfund::contribute(Origin::signed(1), 0, 101));
...
fn withdraw_works() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(SimpleCrowdfund::contribute(Origin::signed(1), 0, 100));
		assert_ok!(SimpleCrowdfund::contribute(Origin::signed(2), 0, 200));
		assert_ok!(SimpleCrowdfund::contribute(Origin::signed(3), 0, 300));
		...
		assert_ok!(SimpleCrowdfund::withdraw(Origin::signed(1), 0));
		assert_eq!(Balances::free_balance(1), 999);

		assert_ok!(SimpleCrowdfund::withdraw(Origin::signed(2), 0));
		assert_eq!(Balances::free_balance(2), 2000);

		assert_ok!(SimpleCrowdfund::withdraw(Origin::signed(3), 0));
...
fn withdraw_handles_basic_errors() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			SimpleCrowdfund::withdraw(Origin::signed(1), 0),
		...
		assert_noop!(
			SimpleCrowdfund::withdraw(Origin::signed(2), 0),
			Error::<Test>::NoContribution
		);
		// Cannot withdraw from a non-existent fund
		assert_noop!(
			SimpleCrowdfund::withdraw(Origin::signed(1), 1),
			Error::<Test>::InvalidIndex
		);
	});
}
...
fn dissolve_handles_basic_errors() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			SimpleCrowdfund::dissolve(Origin::signed(1), 1),
			Error::<Test>::InvalidIndex
		);
		// Cannot dissolve an active fund
		assert_noop!(
			SimpleCrowdfund::dissolve(Origin::signed(1), 0),
...
fn dispense_works() {
	new_test_ext().execute_with(|| {
		// Set up a crowdfund
		assert_ok!(SimpleCrowdfund::create(Origin::signed(1), 20, 1000, 9));
		// Transfer fee is taken here
		assert_ok!(SimpleCrowdfund::contribute(Origin::signed(1), 0, 100));
		assert_ok!(SimpleCrowdfund::contribute(Origin::signed(2), 0, 200));
		assert_ok!(SimpleCrowdfund::contribute(Origin::signed(3), 0, 300));
		assert_ok!(SimpleCrowdfund::contribute(Origin::signed(3), 0, 400));
...
fn dispense_handles_basic_errors() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			SimpleCrowdfund::dispense(Origin::signed(1), 1),
			Error::<Test>::InvalidIndex
		);
		// Cannot dispense an active fund
		assert_noop!(
			SimpleCrowdfund::dispense(Origin::signed(1), 0),
			Error::<Test>::FundStillActive
		);
